...
В первой части я специально сделаю достаточно большой load factor, чтобы наглядно показать диаграммы распределения разных хеш-функций. А во второй части load factor будет уменьшен для увеличения скорости.
...
В качестве объектов я буду использовать строки из трагедии Шекспира "Гамлет". Уникальных слов здесь 4680.*
...
### Оптимизация 4. Уменьшение load factor

Попробуем ускорить программу увеличением размера таблицы. Тогда в списках будет меньше элементов и поиск будет происходить быстрее. Раньше размер был 997, сделаем его 3557.
...
Уменьшение load factor   | 3.87                 | 1.14    

Таким образом, поиск в хеш-таблице ускорен в ```3.87``` раз. Наиболее эффективной оптимизацией является использование SIMD инструкции для сравнения строк.
