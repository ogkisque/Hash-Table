...
Функция на асме      | 1.41                 | 1.41                         
Ассемблерная вставка | 1.43                 | 1.02
SIMD инструкции      | 3.39                 | 2.37

Таким образом, поиск в хеш-таблице ускорен в ```3.39``` раз без изменения каких-либо алгоритмов. Наиболее эффективной оптимизацией является использование SIMD инструкции для сравнения строк.

*P.S. Можно также ускорить поиск путём увеличения размера хеш-таблицы, однако максимальный выигрыш будет 1.2 да и это не является объектом изучения этого проекта.*
